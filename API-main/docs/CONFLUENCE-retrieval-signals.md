...
| `CLUSTER_ACTIVITY_TABLE`, `CLUSTER_MEMBER_TABLE`, `CLUSTER_MAP_TABLE`, `CLUSTER_CLUSTER_TABLE`, `CLUSTER_ALL_RESULTS_TABLE` | `SCENARIO_6_ACTIVITY_CODE_DETAILS_ARRAY`, `SCENARIO_6_MEMBER`, `SCENARIO_6_MAP`, `SCENARIO_6_CLUSTER`, `all_results` | Table names |
...
| Cluster-selection / transaction-selection / BigQuery failure during a request | 200 | Normal envelope; the failure is logged at `ERROR` (`Cluster selection failed…`, `Document-class lookup failed…`, `Record-type transaction pipeline failed`) and the affected record types carry whatever the later stages coded (vocabulary CUIs, model fallback) — `Record types uncoded after every stage` (WARNING) if nothing could |
...
| `temporal` | first non-default window in pipeline order: vocabulary name, CUIs, formula | **same shape**; every window the pipeline resolved is in it — `name` / `formula` of the containing (widest) window, `coding` = the CUIs of all distinct windows, containing window first; a single window is exactly the /v1 projection |
