...
> `/v2/retrieval-signals` keeps the `/v1` request and adds an **Action Finder gate** (optional `actions`); its **record-type CUIs come from the link-code pipeline** instead of the JSON vocabulary, and its **`temporal` is a list** with one entry per distinct window (service 1.5.16). See the **/v2** section further down. `/v3/retrieval-signals` is `/v2` with the temporal step in **canonical mode** — the temporal vocabulary is indexed in-process instead of being pasted into the prompt, every candidate gets its own window with `basis` and `rationale`, and queries with no time expression get a window inferred from the candidate's clinical nature. See the **/v3** section at the end. Everything up to the /v2 section describes `/v1` and applies unchanged to `/v2` and `/v3` unless stated there.
...
| `record_types[].coding` | `record_type_name_to_cui.json` vocabulary, matched by the contextual-environment prompt (`record_type_matches`) | The link-code pipeline: record-type name (+ tag names as context) → cluster selection → activities → Stage 1 → documents → Stage 2 → document classes → Stage 3 → node ids. Empty when the pipeline finds nothing. The record-type vocabulary is never sent to the model (no `KNOWN RECORD TYPES` block). |
| `temporal` | one object: the first non-default window | **a list**: one `{name, formula, coding}` per distinct window the query resolved, in pipeline order (first = the `/v1` window); windows with the same formula are one entry with the CUIs of both; the default window only when nothing else was resolved; `formula` always `[start, end]` (a one-term `REF_POINT - 1Y` → `[REF_POINT, REF_POINT - 1Y]`); a temporal hint → a one-item list |
| `tags` | as /v1 | identical |
...
| New failure | — | `503 cluster_service_unavailable` when the cluster dependency is not configured (checked before any model call). A dependency failing *during* a request is logged and leaves that record type uncoded — never an error response |
...
Record-type names come from the nature breakdown (union of intent-level `retrieval_signals.record_types`, relevance order, de-duplicated, `snake_case` normalised to display form), as production `/v1` generates them; the model is never asked to match them to a vocabulary. Their codes come from the standalone link-code pipeline and nothing else. For each record type, with the query's tag names joined by `", "` as the context (the name itself when there are no tags):
...
Payloads and BigQuery queries are the script's own. A step that finds nothing ends the run for that record type: `coding: []`, and the `Record-type transaction pipeline` log line says which step stopped it. Record types run concurrently; the whole coding step runs against `RECORD_TYPE_RESOLUTION_TIMEOUT_SECONDS` (210 s) — when it runs out the pipeline stops where it is (classes already confirmed in Stage 3 are kept; outcome `deadline reached …`). Raise it together with the request timeout in front of the service if that shows up in the logs. Hints are merged afterwards with the same precedence as `/v1`.
...
| `RECORD_TYPE_RESOLUTION_TIMEOUT_SECONDS` / `TRANSACTION_REQUEST_TIMEOUT_SECONDS` / `CLUSTER_REQUEST_TIMEOUT_SECONDS` | `210` / `300` / `120` | Budget for coding all record types of a query; per-call timeouts (the script's) |
...
- **Cost/latency:** with `actions` supplied each query costs **one extra LLM call** (the gate) ahead of the four pipeline calls; each record type then runs the link-code pipeline (one cluster-selection call, three BigQuery queries, three rounds of transaction-selection calls), all record types of a query concurrently. Gated-out queries cost only the gate call. Queries are still processed sequentially.
- **Migration:** clients can switch from `/v1` to `/v2` without changing their request; in the response `temporal` is a list of `{name, formula, coding}` instead of one object, and a gated-out query returns the skip block (`skipped: true`) when they opt into `actions`.
...
| `temporal` | the list: one entry per distinct window | **the same list**, each entry a vocabulary name, its CUIs and its `[start, end]` formula |
| Response schema | `/v2` | **`/v2`, unchanged** — nothing added per query or in `details`; `window` / `basis` / `rationale` are matching aids that go to the `Temporal inference` log |
| Record types | names from the nature breakdown, codes from the link-code pipeline | identical |
| New failure | — | `503 temporal_index_unavailable` when the vocabulary could not be indexed at startup (`/v1`, `/v2` unaffected) |
| Contextual-environment prompt size (bundled temporal vocabulary) | ≈82.8K chars / ≈20.7K tokens | ≈17.5K chars / ≈4.4K tokens; the record-type vocabulary is never added to either |
...
- **Cost/latency:** the contextual-environment call — the largest of the four — sends ≈79% fewer prompt characters than `/v1` with the bundled temporal vocabulary (and never the record-type vocabulary); the other three calls are unchanged. Shadow mode adds one legacy contextual-environment call per query while it is on.
...
- **Migration:** a `/v2` client can call `/v3` with the same request and parse the same response — the schema is `/v2`'s.
