...
Configuration back in the repository's own format, with the link-code pipeline's values
built in.

### Changed
- `config/.env.<env>` files are plain `KEY=VALUE` lists again (the 1.3.0 format); the
  explanatory blocks moved out. Each lists `PROJECT_ID`, `GCP_LOCATION`, `MODEL_VERSION`
  (+ the vocabulary keys where they were) and the four record-type keys
  (`CLUSTER_SELECTION_URL`, `CLUSTER_BQ_PROJECT`, `CLUSTER_BQ_DATASET`,
  `TRANSACTION_SELECTION_URL`) with their values.
- `app/config.py` holds every setting with its default and explanation. The record-type
  coding block now carries the standalone pipeline script's values as defaults — project
  `aif-usr-p-ep-cg-wcdm-0767`, dataset `aif_wcdm_osm_pipeline`, location `US`, the
  `SCENARIO_6_*` / `all_results` tables, both service URLs, cluster set
  `loinc_document_v001`, top_k 10 / top_p 20 / combined, model `gemini-2.5-flash`, batch
  25 / 30 workers — so the service runs without anything being filled in. A blank value in
  an env file (`KEY=`) now keeps the default instead of turning the setting off
  (`_env_str`); `CLUSTER_BQ_PROJECT` no longer falls back to `PROJECT_ID`.
- Local identity tokens as the script does: `ALLOW_LOCAL_GCLOUD_AUTH` defaults to true,
  `gcloud auth print-identity-token` is tried with `--audiences` (service-account login)
  and then plain (user login, which refuses `--audiences`); new `GCLOUD_PATH` for a gcloud
  that is not on `PATH`. Cloud Run keeps using the metadata server.
- README / Confluence: configuration section rewritten around the two files.

### Tests
- Unit tests never reach BigQuery (the client is stubbed to fail fast) and run the
  cluster path unless a test turns the pipeline on, since the pipeline is now enabled by
  default. 222 pass.

## [1.5.11] - 2026-09-24

Two decisions taken on the 1.5.10 open points.

### Changed
- **Every temporal window in the response (`/v2`, `/v3`).** The nature breakdown resolves
  a window per intent / candidate, so one query can carry several; the block used to
  keep only the first non-default one. The `temporal` object keeps the /v1 shape and now
  carries all of them: `name` / `formula` are those of the window that contains the
  others (the widest by the span its formula encodes, read through the temporal index;
  first in pipeline order when spans tie or cannot be read) and `coding` holds the CUIs of
  every distinct window, the containing window's first, then pipeline order. One window
  → exactly the /v1 projection; default-named windows are ignored while a resolved one
  exists, as before. Logged per query as `Temporal windows merged` (windows, codes).
  `/v1` is untouched (`project_temporal` refactored over a shared entry reader, same
  output).
- **Record-type codes are the concept id, not the row id.** The transaction pipeline
  returns `node_id` from the results table, which carries a per-row suffix
  (`C3166217-0`, `C3166217-1`); the code is now the part before it (`C3166217`), so one
  concept is one code whichever row selected it and the pipeline's codes share the
  code space of the cluster / catalog / vocabulary stages (`cui_from_node_id`).

### Added
- Tests: all-windows merge (containing window, code order, single window = /v1, default
  only, range windows, no index), node-id suffix stripping and de-duplication across
  rows; endpoint tests updated to the merged object. 222 pass.

## [1.5.10] - 2026-09-24

End-to-end hardening of `/v3` (and `/v2`) after live errors, timeouts and empty
signal blocks. Every finding below was reproduced with a real-startup harness
(only Gemini, the two services and BigQuery faked) before it was fixed.

### Fixed
- **Errors (503 after the model work was done).** A cluster-selection error, timeout or
  invalid JSON — or BigQuery failing in both the pipeline and the catalog — aborted the
  request with `503 cluster_service_unavailable` *after* the four model calls had run, so
  the caller got an error body instead of the signals. `resolve_record_type_cuis` now
  never raises for a dependency failure: each failing stage is logged at `ERROR`
  (`Cluster selection failed…`, `Document-class lookup failed…`,
  `Record-type transaction pipeline failed`) and contributes nothing, the next stages and
  the vocabulary / model fallbacks still run, and the handler keeps the record types'
  vocabulary coding if the resolver ever raises anyway. The 503 remains only for a
  missing configuration, answered before any model call.
- **Timeouts.** The transaction pipeline had no overall bound: per call 300 s × up to 5
  retries with back-off sleeps, per stage, per text (label + 2 aliases), so one hung or
  failing transaction-selection call could hold a request far past the 300 s Cloud Run /
  gunicorn limit (a failing service alone cost ~90 s of retry sleeps per record type).
  New `RECORD_TYPE_RESOLUTION_TIMEOUT_SECONDS` (default 120) budgets the coding of all
  record types of a query: every service call is capped by the time left, no retry sleeps
  past it, the pipeline stops between or inside stages when it runs out (classes already
  confirmed in Stage 3 are kept, logged as `ok (partial…)`), and the remaining stages
  code what is still missing. `TRANSACTION_REQUEST_TIMEOUT_SECONDS` default 300 → 60 (a
  call that has not answered in a minute is retried instead of waited on);
  `TRANSACTION_MAX_WORKERS` default 8 → 30, the standalone pipeline's value, so a stage
  of 12 batches is one round instead of two.
- **Duplicate cluster-selection calls.** The same text was sent to the cluster service by
  the pipeline and again by the fan-out stage; one memoised lookup per text now serves
  both (a failed lookup is a logged empty answer).
- **Empty signal blocks.** When the single contextual-environment call came back
  unparseable or invalid, the query was returned with `record_types: []` and the default
  temporal window — logged as a WARNING, never retried. The call is now retried once
  before that happens (`…, retrying once` / `…, returning empty context` in the log; both
  calls' tokens are counted).
- Log fields: an exception's `details` can no longer overwrite the `error_type` /
  `error` of the log entry that reports it.

### Added
- Document catalog warmed in a background thread at startup (when configured), so the
  first request that needs the cluster stages or the model fallback does not load the
  MEMBER → MAP → CLUSTER join inline; startup log now also shows the resolution budget.
- Tests: deadline capping and retry/sleep behaviour, pipeline stop between stages and
  partial Stage 3, resolver surviving service + BigQuery failures, one cluster call per
  text, contextual-environment retry (success on second call / empty after two), handler
  degrading instead of 503 (replaces the former 503 test).

### Notes
- `TRANSACTION_MAX_WORKERS`, `TRANSACTION_REQUEST_TIMEOUT_SECONDS` and
  `RECORD_TYPE_RESOLUTION_TIMEOUT_SECONDS` are commented defaults in `config/.env.*`;
