...
Retrieval signals `/v2` and `/v3`: `temporal` is a list, and record types are no longer
matched by the model. All other endpoints unchanged.

### POST /v2/retrieval-signals, POST /v3/retrieval-signals — changed since 1.5.15

- **`temporal` is a list** (the one response-schema change):
  `temporal: [{name, formula[], coding[] {system, code}}, ...]` — one entry per
  distinct window the nature breakdown resolved for the query, in pipeline order, so
  the first entry is the window `/v1` returns. Windows with the same formula (two
  vocabulary names for one span, e.g. `3 years` / `In past 3 years`) are one entry with
  the CUIs of both; windows with different formulas are separate entries (1.5.15 folded
  them all into one object under one formula). The default window (`Recent`) is listed
  only when nothing else was resolved. `formula` is `[start, end]` as in 1.5.15. A
  client `temporal` hint is returned as a one-item list.
- **Skip block** — `temporal: []` (was `null`), the same type as an answered query.
- **Record types are never matched by the model.** The record-type vocabulary is no
  longer sent to the model (the `KNOWN RECORD TYPES` block; with a GCS record-type
  vocabulary loaded it made the contextual-environment prompt several hundred thousand
  tokens). Record-type names come from the nature breakdown, as production `/v1`
  generates them; their codes come only from the link-code pipeline (node ids of the
  selected document classes). The prompt size of `/v2` and `/v3` no longer depends on the
  record-type vocabulary.
- **Record-type time limits are the pipeline script's** — each transaction-selection call
  may take 300 s (was 60 s) and the budget for coding a query's record types is 210 s
  (was 120 s). In 1.5.15 record types could come back with empty `coding` because the
  budget ran out (`outcome: deadline reached …` in the `Record-type transaction
  pipeline` log).
- **Unchanged:** request schema, `record_types` and `tags` shape, Action Finder gate,
  errors, `details`.

### POST /v3/retrieval-signals — also changed since 1.5.15

- **Spelled-out numbers match like digits** when a window is resolved to its vocabulary
  name: `last three years` now resolves to `3 years` / `In past 3 years`, as `last 3 years`
  does, instead of an entry that only shares the spelled-out number (`Every three years`,
  a frequency with the same formula).

### GET /v2/retrieval-signals/action-types — unchanged
...
- Defaults changed: `TRANSACTION_REQUEST_TIMEOUT_SECONDS` 300 (was 60),
  `RECORD_TYPE_RESOLUTION_TIMEOUT_SECONDS` 210 (was 120). A `/v2` or `/v3` request is the
  model steps plus at most that budget; keep the request timeout in front of the service
  (Cloud Run default 300 s) above it, and raise both together if the logs show
  `deadline reached`.

### Operations

- `Temporal windows merged` is no longer logged. `Vocabulary coverage` (`/v2`, `/v3`)
  now carries `temporal_windows` (name and formula of every window),
  `temporal_coded` (every window has codes) and `unresolved_temporal` (list of names).
...
- **Record-type coding is the link-code pipeline.** For each record type the model named
  ...
  (`C3166217-0` → `C3166217`; one code per concept). Payloads and BigQuery queries are
  the standalone script's own. As in the script, when a step finds nothing the record
  type has no codes (`coding: []`, logged with the step that stopped it) — there is no
  other source of codes: the `record_type_name_to_cui.json` vocabulary and the 1.4.0
  `MEMBER → MAP → CLUSTER` lookup are no longer used by `/v2`.
- **Every temporal window is in the response.** The nature breakdown resolves a window
  ...
  `name` / `formula` of the window that contains the others — a window from the
  reference point (`[REF_POINT, REF_POINT - X]`) reaching farthest back; a range such as
  an age band (`[REF_POINT - 55Y, REF_POINT - 59Y]`) names it only when there is nothing
  else — and `coding` = the CUIs of every distinct window, the containing window's
  first. One window is exactly the `/v1` value. A client `temporal` hint still overrides.
- **`temporal.formula` always has two terms**, `[start, end]`, as every entry of the
  bundled `temporal_name_to_cui.json`: a one-term formula — from a vocabulary file that
  stores a window as one string (`"formula": "REF_POINT - 3M"`, as the AIF GCS file
  does) or from a client hint (`"REF_POINT - 1Y"`) — is returned as
  `["REF_POINT", "REF_POINT - 3M"]`; a hint without a formula gets the vocabulary
  entry's formula for its name, in the same shape. (`/v1` unchanged: it returns the
  vocabulary's formula as stored.)
- **Coding dependencies never produce an error response.** A cluster-selection,
  ...
  affected record type has no codes; the response stays `200` in the normal shape.
  `503 cluster_service_unavailable` is now only answered when the pipeline is not
  configured, before any model call.
...
- **Signals are asked for twice before being given up.** The contextual-environment
  ...
  be parsed or validated, or is JSON but not an object; only two consecutive failures
  return a query with empty `record_types` and the default window. (`/v1` keeps its
  single call.)
...
- New settings: `TRANSACTION_SELECTION_URL`, `TRANSACTION_SELECTION_MODEL`,
  ...
  `RECORD_TYPE_RESOLUTION_TIMEOUT_SECONDS`, `CLUSTER_ACTIVITY_TABLE`,
  `CLUSTER_ALL_RESULTS_TABLE` (+ `_DOC_COLUMN`, `_CLASS_COLUMN`, `_CODE_COLUMN`),
  `CLUSTER_SELECTION_MAX_WORKERS`, `CLUSTER_MEMBER_NAME_KEYS`, `GCLOUD_PATH`; `/v3`: `TEMPORAL_INDEX_SIMILARITY_PROVIDER`, `TEMPORAL_EMBEDDING_MODEL`,
...
- Removed (1.4.0 settings of the `MEMBER → MAP → CLUSTER` lookup, no longer used):
...
- Startup: `Temporal index built` (entries, distinct windows, unit codes),
  `Record-type pipeline configured for /v2 and /v3 retrieval signals` (URLs, project,
  dataset, tables, batch size, workers, budget).
...
- Warnings / errors worth an alert: `Record types the pipeline could not code`,
