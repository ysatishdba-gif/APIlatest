...
- **`temporal.formula` always has two terms**, `[start, end]`, as every entry of
  `temporal_name_to_cui.json`: a one-term formula (e.g. a client hint
  `"REF_POINT - 1Y"`) is returned as `["REF_POINT", "REF_POINT - 1Y"]`; a hint without
  a formula gets the vocabulary entry's formula for its name. (`/v1` unchanged.)
