...
| `RECORD_TYPE_RESOLUTION_TIMEOUT_SECONDS` | Time budget for coding all record types of one query (pipeline + cluster stages; default 120). Every service call is capped by what is left; when it runs out the pipeline stops and the vocabulary/model fallbacks code the rest. Keep it inside the request timeout in front of the service (Cloud Run / gunicorn 300 s) together with the model steps | Optional |
| `TRANSACTION_MAX_WORKERS` / `TRANSACTION_REQUEST_TIMEOUT_SECONDS` | Concurrent transaction-selection batches per record type (default 30, as the standalone pipeline) and the per-call timeout (default 60 s; a call that has not answered by then is retried) | Optional |
...
Permissions for the service account: invoker on the cluster-selection service, BigQuery Data Viewer + Job User on the dataset, Vertex AI user, and read on the GCS vocabularies. Startup logs `Document-cluster record-type resolution configured` (with the URL, project and dataset it will use) or a `WARNING` naming the missing settings, and `Document catalog loaded` with the member count once the catalog is warmed (a background load at startup; otherwise the first request that needs it).
