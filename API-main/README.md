...
| `CLUSTER_ACTIVITY_TABLE`, `CLUSTER_MEMBER_TABLE`, `CLUSTER_MAP_TABLE`, `CLUSTER_CLUSTER_TABLE`, `CLUSTER_ALL_RESULTS_TABLE` | `SCENARIO_6_ACTIVITY_CODE_DETAILS_ARRAY`, `SCENARIO_6_MEMBER`, `SCENARIO_6_MAP`, `SCENARIO_6_CLUSTER`, `all_results` |
...
| `RECORD_TYPE_RESOLUTION_TIMEOUT_SECONDS`, `TRANSACTION_REQUEST_TIMEOUT_SECONDS`, `CLUSTER_REQUEST_TIMEOUT_SECONDS` | `120`, `60`, `120` — the request-path time limits (see the pipeline description below); keep the budget inside the request timeout in front of the service (Cloud Run / gunicorn 300 s) |
| `RECORD_TYPE_RESOLUTION_MODE` | `auto` (pipeline when its URL is set) — `transactions` / `cluster` force one path |
...
Permissions for the service account: invoker on the cluster-selection and transaction-selection services, BigQuery Data Viewer + Job User on the dataset, Vertex AI user, and read on the GCS vocabularies. Startup logs `Document-cluster record-type resolution configured` (with the URLs, project and dataset it will use) and `Document catalog loaded` with the member count once the catalog is warmed (a background load at startup; otherwise the first request that needs it).

### Document-cluster record types + Action Finder (`/v2/retrieval-signals`)

`/v2/retrieval-signals` does **not** use the record-type JSON vocabulary. Record-type names still come from the pipeline, but their CUIs are resolved **per request** through the LOINC document cluster: the name (with the query's tag names as context) is sent to the cluster-selection service, and the returned member document names are mapped to document classes and CUIs in BigQuery. The document catalog is warmed at startup; the settings have working defaults (see Configuration above), and only a setting forced blank in code would make `/v2` answer `503 cluster_service_unavailable`.
...
2. **Document-cluster record types** — `record_types` CUIs come from the cluster-selection service + BigQuery lookup (see [Document-cluster record types](#document-cluster-record-types--action-finder-v2retrieval-signals)); the `record_type_name_to_cui.json` vocabulary is not consulted. When that dependency is not configured the endpoint returns `503` with `error: "cluster_service_unavailable"` before any model call; a failure of it during a request is logged and degrades the coding (vocabulary / model fallbacks), never the response.
...
| Output `temporal` | a vocabulary name, its CUIs, its formula (first non-default window in pipeline order) | **the same shape**, carrying every window the nature breakdown resolved: the pipeline resolves a window per intent / candidate, so `name` / `formula` are those of the window that contains the others (the widest by the span its formula encodes) and `coding` holds the CUIs of every distinct window, the containing one's first (`/v2` does the same; `/v1` keeps its first-window rule). One window → exactly the /v1 projection. Logged per query as `Temporal windows merged` |
