...
Record-type **names** come from the `/v1` contextual-environment prompt (`KNOWN RECORD TYPES` block included), so they are generated exactly as `/v1`. Their **codes** come from the standalone link-code pipeline and nothing else. For each record type, with the query's tag names joined by `", "` as the context (the name itself when there are no tags):
...
- Time: the coding of all record types of a query runs against `RECORD_TYPE_RESOLUTION_TIMEOUT_SECONDS` (120 s): every call is capped by the time left, no retry sleeps past it, and when it runs out the pipeline stops where it is (classes already confirmed in Stage 3 are kept). Per-call timeouts: `TRANSACTION_REQUEST_TIMEOUT_SECONDS` (60), `CLUSTER_REQUEST_TIMEOUT_SECONDS` (120).
...
| Output `temporal` | a vocabulary name, its CUIs, its formula (first non-default window in pipeline order) | **the same shape**, carrying every window the nature breakdown resolved: the pipeline resolves a window per intent / candidate, so `name` / `formula` are those of the window that contains the others (a window from the reference point, `[REF_POINT, REF_POINT - X]`, outranks a range such as an age band; among those the one reaching farthest back) and `coding` holds the CUIs of every distinct window, the containing one's first (`/v2` does the same; `/v1` keeps its first-window rule). One window → exactly the /v1 projection. `formula` always has two terms as in `temporal_name_to_cui.json` — a one-term formula (a vocabulary file that stores the window as one string, as the AIF GCS file does, or a client hint `REF_POINT - 1Y`) is returned as `["REF_POINT", "REF_POINT - 1Y"]`. Logged per query as `Temporal windows merged` |
