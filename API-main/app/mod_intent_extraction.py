...
class ContextualIntentPipeline:
    ...
    async def build_context_async(
        ...
        for attempt in range(1, attempts + 1):
            ...
            if data.get("_parsing_error"):
                failure = {
                    "message": "Contextual environment parsing failed",
                    "error_message": data.get("_error_message"),
                    "is_truncated": data.get("_is_truncated"),
                }
            else:
                try:
                    ...
                    failure = {
                        "message": "Contextual environment validation failed",
                        "validation_error_count": len(e.errors()),
                    }
            if failure is None:
                break
            ...
            if self.lg:
                self.lg.log_struct(
                    ...
                    + (", retrying once" if retrying else ", returning empty context"),
                    structured_data={
                        **self.base_log,
                        **failure,
                        "attempt": attempt,
                        "fallback": not retrying,
                    },
                    severity="WARNING",
                )
        if context is None:
