...
# Record types for /v2 and /v3 are coded per request by the link-code
# pipeline (cluster selection -> activities -> Stage 1 -> documents -> Stage 2
# -> document classes -> Stage 3 -> node ids). Nothing is loaded at startup;
# the configuration is logged so the services and tables in use are visible.
if document_cluster.is_configured():
    lg.log_struct(
        message="Record-type pipeline configured for /v2 and /v3 retrieval signals",
        structured_data={
            **base_log,
            "cluster_selection_url": config.CLUSTER_SELECTION_URL,
            "transaction_selection_url": config.TRANSACTION_SELECTION_URL,
            ...
            "transaction_selection_model": config.TRANSACTION_SELECTION_MODEL,
            "batch_size": config.TRANSACTION_BATCH_SIZE,
            "max_workers": config.TRANSACTION_MAX_WORKERS,
            "record_type_resolution_timeout_seconds": (
                config.RECORD_TYPE_RESOLUTION_TIMEOUT_SECONDS
            ),
        },
        severity="INFO",
    )
    ...
    lg.log_struct(
        message="Record-type pipeline not configured; /v2 and /v3 retrieval signals will answer 503",
        structured_data={
            **base_log,
...
#
# Same request contract as /v1 with these differences:
...
#   2. Record types are coded through the link-code pipeline (cluster
#      selection + BigQuery + transaction selection), not the
#      record_type_name_to_cui.json vocabulary. The contextual-environment
#      prompt therefore runs without the KNOWN RECORD TYPES block
#      (include_record_type_matching=False).
#   3. `temporal` is a list: one entry per distinct window.
# /v1 is untouched.
# ---------------------------------------------------------------------------
...
@app.post(
    ...
    description=(
        ...
        "the local record-type vocabulary. (3) `temporal` is a list: one "
        "{name, formula, coding} per distinct window resolved for the query, the "
        "formula always [start, end]. record_types, temporal, and tags may be "
        "supplied as optional hints exactly as in /v1 (a temporal hint is returned "
        "as the one-item list)."
    ),
    response_model=RetrievalSignalsResponse,
)
...
@app.post(
    ...
    description=(
        "Same request, response schema and per-query block as /v2 "
        ...
        "narrowest vocabulary window that contains it). `temporal` is the /v2 list: "
        "one entry per distinct window, each with its vocabulary name, CUIs and "
        "[start, end] formula."
    ),
    response_model=RetrievalSignalsResponse,
)
...
async def _run_retrieval_signals(
    ...
    async def _process_query(q) -> None:
        ...
        v2_result = await pipeline.run_v2_async(
            ...
            enable_retrieval_signals=True,
            location=resolved_location,
            # Record-type NAMES come from the nature breakdown's own
            # instructions, as production generates them; the record-type
            # vocabulary is never sent to the model. Their CODES come from the
            # link-code pipeline below.
            include_record_type_matching=False,
            temporal_mode=temporal_mode,
        ...
        # /v1 projection of the block (same record-type names, tags with
        # names and topics); record-type codes and temporal are set below.
        block = project_query_signals(
            query_id=q.id,
            text=q.text,
            v2_result=v2_result,
            record_type_vocab=None,  # names only; codes come from the pipeline
            temporal_vocab=temporal_vocab,
            tag_vocab=None,
        )
        # Every temporal window resolved for the query, one entry per distinct
        # formula (the first is the window /v1 returns).
        block["temporal"] = project_temporal_windows(v2_result, temporal_vocab)
        # Record types: names from the nature breakdown; codes from the
        # link-code pipeline, run per record type with the tag names as
        # context (never from the model or the record-type vocabulary). The
        # lookup is blocking I/O (HTTP + BigQuery), so it runs off the event
        # loop. A record type the pipeline could not code keeps an empty
        # coding — as the standalone script records a pair without classes.
        ...
        try:
            ...
            # The resolver logs and swallows every dependency failure; if it
            # ever raises anyway, the record types keep their names with no
            # codes rather than turning the finished model work into an error.
            details = getattr(e, "details", None) or {}
            lg.log_struct(
                message="Record-type pipeline failed; record types left uncoded",
                structured_data={
                    **details,
                    **base_log,
                    "query_id": q.id,
            ...
            block["record_types"] = [
                {"name": rt["name"], "coding": []}
                for rt in block.get("record_types") or []
            ]
        finally:
            cluster_seconds.append(time.perf_counter() - cluster_started)
        uncoded = [rt["name"] for rt in block["record_types"] if not rt.get("coding")]
        if uncoded:
            lg.log_struct(
                message="Record types the pipeline could not code",
                structured_data={
                    **base_log,
                    "query_id": q.id,
                    "record_types": uncoded,
                },
                severity="WARNING",
            )

        # Client hints: same precedence as /v1 (hints first; a temporal hint
        # replaces the derived windows), the hint's formula in the same
        # two-term shape.
        block = merge_hints(
            block,
            [rt.model_dump() for rt in q.record_types] if q.record_types else None,
        ...
        if q.temporal:
            hint = q.temporal.model_dump()
            hint["formula"] = two_point_formula(
                hint.get("formula"), hint.get("name"), temporal_vocab
            )
            block["temporal"] = [hint]
        ...
        lg.log_struct(
            ...
            structured_data={
                **base_log,
                ...
                "unresolved_record_types": unresolved,
                "temporal_windows": [
                    {"name": w.get("name"), "formula": w.get("formula")}
                    for w in windows
                ],
                "temporal_coded": all(w.get("coding") for w in windows),
                "unresolved_temporal": [
                    w.get("name") for w in windows if not w.get("coding")
                ],
            },
            severity="INFO",
        )
