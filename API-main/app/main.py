...
@app.post(
    ...
    description=(
        ...
        "object as /v1 with a vocabulary name, its CUIs and its formula, carrying "
        "every window resolved for the query: the containing window names "
        "it and the CUIs of all distinct windows are in its coding."
    ),
    response_model=RetrievalSignalsResponse,
)
...
async def _run_retrieval_signals(
    ...
    async def _process_query(q) -> None:
        ...
        # /v1 projection of the block (same shape, same record-type names,
        # tags with names and topics only); the record-type codes and the
        # temporal object are then replaced below.
        block = project_query_signals(
            query_id=q.id,
            text=q.text,
            v2_result=v2_result,
        ...
        # Every window the nature breakdown resolved for the query, in the
        # one /v1-shaped temporal object (name/formula of the containing
        # window, coding of all of them). /v1 keeps its first-window rule.
        block["temporal"] = project_temporal_all_windows(
            v2_result, temporal_vocab, pipeline._temporal_index
        )
        merged_windows = (block["temporal"] or {}).pop("_windows", None)
        if merged_windows:
            lg.log_struct(
                message="Temporal windows merged",
                structured_data={
                    **base_log,
                    "query_id": q.id,
                    "temporal_mode": temporal_mode,
                    "name": block["temporal"]["name"],
                    "windows": merged_windows,
                    "codes": [c["code"] for c in block["temporal"]["coding"]],
                },
                severity="INFO",
            )
        # Record types: names exactly as /v1; codes from the link-code
        # pipeline, run per record type with the tag names as context. The
        ...
        # The temporal formula in the vocabulary file's shape, [start, end]
        # (a one-term hint such as "REF_POINT - 1Y" becomes
        # ["REF_POINT", "REF_POINT - 1Y"]).
        if block.get("temporal"):
            block["temporal"]["formula"] = two_point_formula(
                block["temporal"].get("formula"),
                block["temporal"].get("name"),
                temporal_vocab,
            )
