...
if document_cluster.is_configured():
    ...
    # Warm the document catalog (MEMBER -> MAP -> CLUSTER join) off the
    # startup path so the first request that needs the cluster stages or the
    # model fallback does not pay for the load; a failure is logged and the
    # first request loads it instead.
    if config.CLUSTER_DOCUMENT_CATALOG_TTL_SECONDS > 0:
        threading.Thread(
            target=document_cluster.get_document_catalog,
            name="document-catalog-warmup",
            daemon=True,
        ).start()
else:
    lg.log_struct(
...
@app.post(
    ...
    description=(
        ...
        "every window resolved for the query: the containing (widest) window names "
...
async def _run_retrieval_signals(
    ...
    async def _process_query(q) -> None:
        ...
        try:
            ...
            # The resolver degrades stage by stage and should not raise; if
            # it ever does, the record types keep their /v1 vocabulary coding
            # and the fallbacks below still run. The model work is done by
            # now — a coding dependency must not turn it into an error body.
            details = getattr(e, "details", None) or {}
            lg.log_struct(
                message="Document-cluster record-type resolution failed; "
                "keeping vocabulary coding",
                structured_data={
                    **details,
                    **base_log,
                    "query_id": q.id,
            ...
            block["record_types"] = [
                {"name": rt["name"], "coding": list(rt.get("coding") or [])}
