...
def resolve_record_type_cuis(
    ...
    This never raises for a dependency failure: a stage that fails (service
    error, timeout, BigQuery error) is logged and contributes nothing, and
    the next stage — and the caller's vocabulary and model fallbacks — still
    run, so an outage degrades the coding, never the response. The whole
    resolution works against RECORD_TYPE_RESOLUTION_TIMEOUT_SECONDS.
    """
    ...
    budget = float(getattr(config, "RECORD_TYPE_RESOLUTION_TIMEOUT_SECONDS", 120) or 0)
    deadline = time.monotonic() + budget if budget > 0 else None

    # One cluster-selection call per distinct text for the whole resolution:
    # the pipeline and the fan-out below share it, and a failed call is a
    # logged empty answer, not an exception.
    probable_lock = threading.Lock()
    probable_memo: dict[str, list[str]] = {}

    def probable(text: str, ctx: str | None = context) -> list[str]:
        key = _norm(text)
        with probable_lock:
            if key in probable_memo:
                return probable_memo[key]
        try:
            docs = get_probable_documents(text, ctx, timeout=tx.remaining(deadline))
        except Exception as e:  # noqa: BLE001 — the service never fails the request
    ...
    if tx.is_enabled():
        ...
        with ThreadPoolExecutor(max_workers=fanout) as executor:
            for name, hits in zip(
                names,
                executor.map(
                    lambda n: _pipeline_for(
                        tx, n, texts_by_name[n], context, deadline, probable
                    ),
                    names,
                ),
                strict=True,
    ...
    with ThreadPoolExecutor(max_workers=workers) as executor:
        results = list(executor.map(lambda j: probable(j[1]), jobs))
    probable_docs: dict[str, list[str]] = {name: [] for name in names}
    for (name, _text), docs in zip(jobs, results, strict=True):
        for d in docs:
            if d not in probable_docs[name]:
                probable_docs[name].append(d)

    # Everything worth looking up: member documents the service named, plus
    # the record-type name and its aliases themselves.
    all_docs: list[str] = []
    for name in names:
        for d in probable_docs[name] + texts_by_name[name]:
            if d not in all_docs:
                all_docs.append(d)
    rows: list[dict[str, str]] = []
    if all_docs:
        try:
            rows = get_document_classes(all_docs)
        except Exception as e:  # noqa: BLE001 — BigQuery never fails the request
            details = getattr(e, "details", None) or {}
            _log(
    ...
    for name in names:
        if name in pipeline_hits:
            ...
            continue
        doc_set = {_norm(d) for d in probable_docs[name]}
...
def _pipeline_for(
    ...
    probable_documents: Callable[[str, str | None], list[str]] | None = None,
) -> list[dict[str, Any]]:
    """Run the transaction pipeline for the record-type label, then for its
    aliases until one of them selects document classes — while the
    resolution deadline allows."""
    for text in texts:
        left = tx.remaining(deadline)
        if left is not None and left <= 0:
            _log(
                "Record-type transaction pipeline skipped: resolution deadline reached",
                {"record_type": name, "text": text},
            )
            break
        try:
            hits = tx.resolve_via_transactions(
                text, context, deadline=deadline, probable_documents=probable_documents
            )
