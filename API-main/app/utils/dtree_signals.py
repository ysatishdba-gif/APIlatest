...
def project_temporal_all_windows(
    ...
    `name` / `formula` are those of the window that CONTAINS the others (the
    widest, by the span its formula encodes; the first in pipeline order when
    spans tie or cannot be read), and `coding` carries the CUIs of every
    distinct window, the containing window's first, then pipeline order.
    ...
    order = sorted(
        ...
        key=lambda i: (
            ...
            -(spans[i][1] if spans[i] else 0.0),  # farthest edge first
            spans[i][0] if spans[i] else 0.0,  # then the nearest near edge
