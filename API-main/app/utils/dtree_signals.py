...
def project_temporal_windows(
    v2_result: dict[str, Any],
    temporal_vocab: TemporalVocab | None = None,
) -> list[dict[str, Any]]:
    """/v2 and /v3: every temporal window the nature breakdown resolved for
    the query, one `{name, formula, coding}` per distinct formula.

    Windows with the same formula (two vocabulary names for the same span,
    e.g. "3 years" and "In past 3 years") are one entry with the codes of
    both; different formulas stay separate. Order is the pipeline's, so the
    first entry is the window /v1 returns. Default-named windows are left
    out while a resolved one exists; with none, the single /v1 fallback
    window. Every formula has two terms (see two_point_formula).
    """
    ...
    for entry, is_default in _temporal_entries(v2_result, temporal_vocab):
        if is_default:
            continue
        formula = two_point_formula(entry["formula"], entry["name"], temporal_vocab)
        key = (
            tuple(formula)
            if isinstance(formula, list) and formula
            else ("name", _fold_label(entry["name"] or ""))
        )
        window = by_key.get(key)
        if window is None:
            window = {"name": entry["name"], "formula": formula, "coding": []}
            by_key[key] = window
            windows.append(window)
        for c in entry["coding"]:
            if c not in window["coding"]:
                window["coding"].append(dict(c))
    if windows:
        return windows
    fallback = project_temporal(v2_result, temporal_vocab)
    fallback["formula"] = two_point_formula(
        fallback.get("formula"), fallback.get("name"), temporal_vocab
    )
    return [fallback]
