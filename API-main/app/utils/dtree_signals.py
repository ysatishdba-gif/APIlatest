...
def two_point_formula(
    ...
    - one offset term (``"REF_POINT - 1Y"``, a client hint): the window from
      the reference point back to it, ``["REF_POINT", "REF_POINT - 1Y"]``;
    ...
    if (
        ...
        for entry in temporal_vocab.codes_for_name(temporal_vocab.name_from_text(name)):
            entry_terms = [t for t in formula_terms(entry.formula) if t and t.strip()]
