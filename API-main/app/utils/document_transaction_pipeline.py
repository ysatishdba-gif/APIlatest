...
the pipeline stops and reports how far it got — the caller's cheaper stages
then code what is still missing. Nothing here raises to the caller.
"""
...
def resolve_via_transactions(
    ...
    *,
    ...
    ``probable_documents`` lets the caller supply (and memoise) the
    cluster-selection lookup so the same text is never sent twice.
    """
    ...
    # 1. cluster selection
    lookup = probable_documents or cluster.get_probable_documents
    probable = lookup(search_document_name, search_context)
