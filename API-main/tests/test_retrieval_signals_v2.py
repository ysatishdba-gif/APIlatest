...
def cluster_configured(monkeypatch):
    ...
    # These tests describe the direct cluster -> catalog path; the transaction
    # pipeline (on by default) is exercised in test_transaction_pipeline.py.
    ...
    def test_resolve_record_type_cuis_default_path_batches_bigquery(
        ...
        with (
            patch.object(
                ...
                side_effect=lambda n, c, timeout=None: probable.get(n, []),
            ),
            patch.object(cluster, "get_document_classes", return_value=rows) as bq,
        ):
            out = cluster.resolve_record_type_cuis(
...
class TestEndpointV2:
    ...
    def test_cluster_resolution_failure_degrades_never_503(self, client):
        """A record-type dependency failure after the model work is done keeps
        the /v1 envelope: the record types keep their vocabulary coding (here
        none: no vocabulary loaded), the failure is logged, the response is
        200 with every block key present."""
        with (
            ...
            patch.object(config, "RECORD_TYPE_LLM_FALLBACK", False),
        ...
        assert [rt["name"] for rt in block["record_types"]] == [
            "Radiology Report",
            "Clinical Note",
        ]
        assert block["temporal"]["name"] == "Last One Year"
        errors = [
            c.kwargs
