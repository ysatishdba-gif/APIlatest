...
patchers = [
    ...
    # The record-type coding is configured out of the box (real project,
    # dataset and service URLs are the defaults), so BigQuery must never be
    # reachable from a unit test: any unpatched use fails at once instead of
    # waiting on the network. Tests that need a client patch it themselves.
    patch(
        "app.utils.context_lonic_document_cluster.get_bq_client",
        side_effect=RuntimeError("BigQuery is not available in unit tests"),
    ),
]
...
def _reset_document_catalog(monkeypatch):
    """The document catalog is cached process-wide; tests that fake BigQuery
    must not see another test's catalog. The transaction pipeline (on by
    default: its URL is a real default) is off unless a test turns it on
    (`pipeline_on` in test_transaction_pipeline.py), so the cluster-path
    tests exercise the path they describe."""
    from app import config as _config
    from app.utils import context_lonic_document_cluster as _dc

    monkeypatch.setattr(_config, "TRANSACTION_SELECTION_URL", "")
