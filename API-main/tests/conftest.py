...
def _reset_identity_tokens():
    """Identity tokens are cached process-wide; a test that fakes the token
    source must not see another test's token."""
    from app.utils import context_lonic_document_cluster as _dc

    _dc.reset_auth_cache()
    yield
    _dc.reset_auth_cache()
