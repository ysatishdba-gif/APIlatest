...
class TestTemporalIndex:
    ...
    def test_number_words_compare_as_digits(self):
        """ "last three years" is ranked like "last 3 years": the window names
        come before a name that only shares the spelled-out number."""
        idx = TemporalIndex.from_vocab(
            TemporalVocab(
                {
                    "Every three years": [
                    ...
                    "In past 3 years": [
                        {"cui": "C_PAST_3Y", "formula": ["REF_POINT", "REF_POINT - 3Y"]}
                    ],
                }
            )
        )
        key = idx.key_for("single", 3, "year")
        top = idx.rank_for_key(key, "last three years", 1)
        assert top[0].entry.cui == "C_PAST_3Y"
        assert idx.rank_for_key(key, "every three years", 1)[0].entry.cui == (
            "C_EVERY_3Y"
        )
...
class TestProjections:
    ...
    def test_every_window_is_its_own_entry(self, vocab):
        """/v2 and /v3: temporal is a list, one {name, formula, coding} per
        distinct formula, in pipeline order; windows with the same formula
        (two vocabulary names for one span) are one entry with both codes."""
        res = _v2_result_canonical()
        windows = project_temporal_windows(res, vocab)
        assert windows == [
            {
                "name": "Past 6 Months",
                "formula": ["REF_POINT", "REF_POINT - 6M"],
            ...
            {
                "name": "In past 3 years",
                "formula": ["REF_POINT", "REF_POINT - 3Y"],
                "coding": [{"system": "UMLS", "code": "C_3Y"}],
            },
        ]
        # the first entry is the window /v1 returns
        primary = project_temporal(_v2_result_canonical(), vocab)
        assert windows[0]["name"] == primary["name"]
        assert windows[0]["formula"] == primary["formula"]
        # a range is a different formula: its own entry, as the vocabulary has it
        rng = _v2_result_canonical()
        rng["intents"][0]["retrieval_signals"]["temporal"].append(
            {
                "time_window": "6 to 12 months ago",
                "codes": "C_6_12M",
                "formula": ["REF_POINT - 6M", "REF_POINT - 12M"],
            }
        )
        assert project_temporal_windows(rng, vocab)[-1] == {
            "name": "6 to 12 months ago",
            "formula": ["REF_POINT - 6M", "REF_POINT - 12M"],
            "coding": [{"system": "UMLS", "code": "C_6_12M"}],
        }
        # the same span in the one-string shape (AIF GCS file) joins its entry
        same = _v2_result_canonical()
        same["intents"][0]["retrieval_signals"]["temporal"].append(
            {"time_window": "3 years", "codes": "C_3Y_B", "formula": "REF_POINT - 3Y"}
        )
        assert [
            c["code"] for c in project_temporal_windows(same, vocab)[1]["coding"]
        ] == [
            "C_3Y",
            "C_3Y_B",
        ]
        # default-named windows are left out while a resolved one exists
        mixed = _v2_result_canonical()
        mixed["intents"][0]["retrieval_signals"]["temporal"].insert(
            0, {"time_window": "Recent", "codes": "C0332185"}
        )
        assert project_temporal_windows(mixed, vocab) == windows
        # only default-named windows: the single /v1 window, as a one-item list
        default_only = _v2_result_canonical()
        for t in default_only["intents"][0]["retrieval_signals"]["temporal"]:
            t["time_window"] = "Recent"
            t["codes"] = "C0332185"
        assert project_temporal_windows(default_only, vocab) == [
            {
                "name": "Recent",
                "formula": ["REF_POINT", "REF_POINT - 6M"],
        ...
        # no temporal signal at all: the service fallback, as /v1
        none = _v2_result_canonical()
        none["intents"][0]["retrieval_signals"]["temporal"] = []
        assert project_temporal_windows(none, vocab) == [
            {
                "name": "Recent",
    ...
    def test_aif_vocabulary_single_string_formula(self):
        """The AIF GCS vocabulary stores a window as one string
        ("REF_POINT - 3M"): the index keys it by that formula, and /v2 and /v3
        return it as two terms."""
        tv = TemporalVocab(
            {
                "3 Months": [{"cui": "C1442461", "formula": "REF_POINT - 3M"}],
                "In past 3 years": [{"cui": "C3843792", "formula": "REF_POINT - 3Y"}],
                "55-59": [
                    {
                        "cui": "C3843781",
                        "formula": ["REF_POINT - 55Y", "REF_POINT - 59Y"],
                    }
                ],
            }
        )
        idx = TemporalIndex.from_vocab(tv)
        by = {e.name: e for e in idx.entries}
        assert by["3 Months"].key == idx.key_for("single", 3, "month")
        assert by["3 Months"].key_source == "formula"
        assert two_point_formula(by["3 Months"].formula) == [
            "REF_POINT",
            "REF_POINT - 3M",
        ]
        # a hint with a name and no formula, on this file shape
        assert two_point_formula(None, "3 months", tv) == [
            "REF_POINT",
            "REF_POINT - 3M",
        ]
        assert two_point_formula(None, "55-59", tv) == [
            "REF_POINT - 55Y",
            "REF_POINT - 59Y",
        ]

    def test_two_point_formula_matches_the_vocabulary_shape(self, vocab):
        """/v2 and /v3 return the formula as the JSON has it: [start, end]."""
        assert two_point_formula(["REF_POINT", "REF_POINT - 3Y"]) == [
            "REF_POINT",
            "REF_POINT - 3Y",
        ]
        assert two_point_formula(["REF_POINT - 6M", "REF_POINT - 12M"]) == [
            "REF_POINT - 6M",
            "REF_POINT - 12M",
        ]
        # a one-term hint: from the reference point back to it
        assert two_point_formula(["REF_POINT - 1Y"]) == ["REF_POINT", "REF_POINT - 1Y"]
        assert two_point_formula("REF_POINT - 6M") == ["REF_POINT", "REF_POINT - 6M"]
        assert two_point_formula(["REF_POINT"]) == ["REF_POINT", "REF_POINT"]
        # no formula: the vocabulary entry's own, when the name is known
        assert two_point_formula(None, "In past 3 years", vocab) == [
            "REF_POINT",
            "REF_POINT - 3Y",
        ]
        assert two_point_formula(None, "anything", None) is None
...
class TestEndpointV3:
    def test_200_canonical_projection_and_details(self, client):
        ...
        # /v1 per-query keys; temporal is a list with one entry per formula:
        # the HbA1c window (6 months, two vocabulary names) and the diagnosis
        # window (3 years) stay separate.
        ...
        # Record types are never matched by the model: the names come from the
        # nature breakdown, the codes from the pipeline.
        assert kwargs["include_record_type_matching"] is False

    def test_record_type_the_pipeline_cannot_code_stays_uncoded(self, client):
        """As the standalone pipeline: no codes from the pipeline -> no codes.
        The /v1 vocabulary match of the same name is not used."""
    ...
    def test_v2_response_unchanged(self, client):
        ...
        # /v2 lists every resolved window the same way /v3 does.
        assert [w["name"] for w in q["temporal"]] == [
            "Past 6 Months",
            "In past 3 years",
        ]
        assert run.call_args.kwargs["temporal_mode"] == TEMPORAL_MODE_VOCAB_LIST
        assert run.call_args.kwargs["include_record_type_matching"] is False

    def test_single_string_vocabulary_formula_is_returned_as_two_terms(self, client):
        """Windows resolved from a vocabulary that stores "REF_POINT - 3Y"
        (the AIF GCS file) come back as [REF_POINT, REF_POINT - 3Y] from /v2
        and /v3; /v1 returns them as it always has."""

        async def _mock(text: str, **kwargs):
            res = await _mock_v3(text, **kwargs)
            for t in res["intents"][0]["retrieval_signals"]["temporal"]:
                t["formula"] = t["formula"][-1]
            return res

        body = {"queries": [{"text": ["x"]}]}
        with patch("app.main.pipeline.run_v2_async", side_effect=_mock):
            v3 = client.post(RS_V3, json=body).json()["output"]["queries"][0]
            v2 = client.post(RS_V2, json=body).json()["output"]["queries"][0]
            v1 = client.post(RS_V1, json=body).json()["output"]["queries"][0]
        for out in (v3, v2):
            assert [w["formula"] for w in out["temporal"]] == [
                ["REF_POINT", "REF_POINT - 6M"],
                ["REF_POINT", "REF_POINT - 3Y"],
            ]
        assert v1["temporal"]["formula"] == ["REF_POINT - 6M"]

    def test_v2_and_v3_ask_again_for_an_unusable_answer_v1_does_not(self, client):
        with patch("app.main.pipeline.run_v2_async", side_effect=_mock_v3) as run:
            client.post(RS_V3, json={"queries": [{"text": ["x"]}]})
            assert run.call_args.kwargs["retry_invalid_context"] is True
            client.post(RS_V2, json={"queries": [{"text": ["x"]}]})
            assert run.call_args.kwargs["retry_invalid_context"] is True
            client.post(RS_V1, json={"queries": [{"text": ["x"]}]})
            assert "retry_invalid_context" not in run.call_args.kwargs

    def test_temporal_formula_has_two_terms_like_the_vocabulary(self, client):
        """A one-term formula (the /docs example hint) comes back as
        [REF_POINT, REF_POINT - 1Y] from /v2 and /v3; /v1 is unchanged."""
        hint = {"name": "Last One Year", "formula": ["REF_POINT - 1Y"]}
        body = {"queries": [{"text": ["x"], "temporal": hint}]}
        with patch("app.main.pipeline.run_v2_async", side_effect=_mock_v3):
            v3 = client.post(RS_V3, json=body).json()["output"]["queries"][0]
            v2 = client.post(RS_V2, json=body).json()["output"]["queries"][0]
            v1 = client.post(RS_V1, json=body).json()["output"]["queries"][0]
        # the hint replaces the windows: a one-item list
        assert v3["temporal"] == [
            {
                "name": "Last One Year",
        ...
        assert v2["temporal"] == v3["temporal"]
        assert v1["temporal"]["formula"] == ["REF_POINT - 1Y"]
