...
class TestProjections:
    ...
    def test_all_windows_merge_into_the_v1_shape(self, vocab):
        """/v2 and /v3: the nature breakdown resolves a window per candidate;
        all of them go into the one temporal object — the containing window
        names it, every window's codes are in coding (containing first)."""
        index = TemporalIndex.from_vocab(vocab)
        res = _v2_result_canonical()
        merged = project_temporal_all_windows(res, vocab, index)
        windows = merged.pop("_windows")
        assert merged == {
            "name": "In past 3 years",
            "formula": ["REF_POINT", "REF_POINT - 3Y"],
            "coding": [
                {"system": "UMLS", "code": "C_3Y"},
        ...
        # one window: exactly the /v1 projection, nothing to merge
        one = _v2_result_canonical()
        one["intents"][0]["retrieval_signals"]["temporal"] = one["intents"][0][
            "retrieval_signals"
        ]["temporal"][:1]
        assert project_temporal_all_windows(one, vocab, index) == project_temporal(
            one, vocab
        )
        # only default-named windows: the default, as /v1
        default_only = _v2_result_canonical()
        for t in default_only["intents"][0]["retrieval_signals"]["temporal"]:
            t["time_window"] = "Recent"
            t["codes"] = "C0332185"
        assert project_temporal_all_windows(default_only, vocab, index) == {
            "name": "Recent",
            "formula": ["REF_POINT", "REF_POINT - 6M"],
            "coding": [{"system": "UMLS", "code": "C0332185"}],
        }
        # a range window is contained by the single window whose far edge
        # reaches past it; without an index the first window in pipeline
        # order names the object and the codes are still all there
        ...
        assert project_temporal_all_windows(rng, vocab, index)["name"] == (
            "In past 3 years"
        )
        # a range reaching farther back never names the object while a window
        # from the reference point exists (it would not contain the others);
        # its codes are still in coding
        band = _v2_result_canonical()
        band["intents"][0]["retrieval_signals"]["temporal"].append(
            {
                "time_window": "55-59",
                "codes": "C_BAND",
        ...
        merged_band = project_temporal_all_windows(band, vocab, index)
        assert merged_band["name"] == "In past 3 years"
        assert merged_band["formula"] == ["REF_POINT", "REF_POINT - 3Y"]
        assert "C_BAND" in [c["code"] for c in merged_band["coding"]]
        # only ranges: the range is what there is
        only_ranges = _v2_result_canonical()
        only_ranges["intents"][0]["retrieval_signals"]["temporal"] = [
            {
                "time_window": "6 to 12 months ago",
                "codes": "C_6_12M",
                "formula": ["REF_POINT - 6M", "REF_POINT - 12M"],
        ...
        assert project_temporal_all_windows(only_ranges, vocab, index)["formula"] == [
            "REF_POINT - 6M",
            "REF_POINT - 12M",
        ]
        no_index = project_temporal_all_windows(rng, vocab, None)
        assert no_index["name"] == "Past 6 Months"
        assert [c["code"] for c in no_index["coding"]] == [
            "C_6M_A",
            "C_6M_B",
            "C_3Y",
            "C_6_12M",
...
class TestEndpointV3:
    def test_200_canonical_projection_and_details(self, client):
        ...
        # /v1 per-query block, nothing added: same keys, same temporal shape.
        # Every window the pipeline resolved is in it: the diagnosis window
        # (3 years) contains the HbA1c windows (6 months), so it names the
        # object and every window's codes follow, containing window first.
    ...
    def test_v2_response_unchanged(self, client):
        ...
        # /v2 merges every resolved window the same way /v3 does.
        assert q["temporal"]["name"] == "In past 3 years"
        assert [c["code"] for c in q["temporal"]["coding"]] == [
            "C_3Y",
            "C_6M_A",
            "C_6M_B",
        ]
    ...
    def test_single_string_vocabulary_formula_is_returned_as_two_terms(self, client):
        ...
        assert v3["temporal"]["formula"] == ["REF_POINT", "REF_POINT - 3Y"]
        assert v2["temporal"]["formula"] == ["REF_POINT", "REF_POINT - 3Y"]
    ...
    def test_temporal_formula_has_two_terms_like_the_vocabulary(self, client):
        ...
        with patch("app.main.pipeline.run_v2_async", side_effect=_mock_v3):
            ...
            v2 = client.post(RS_V2, json=body).json()["output"]["queries"][0]
            v1 = client.post(RS_V1, json=body).json()["output"]["queries"][0]
        assert v3["temporal"]["formula"] == ["REF_POINT", "REF_POINT - 1Y"]
        assert v2["temporal"]["formula"] == ["REF_POINT", "REF_POINT - 1Y"]
