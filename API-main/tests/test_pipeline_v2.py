...
class TestContextualEnvironmentRetry:
    """Every record type and temporal window comes out of the one contextual
    environment call: for /v2 and /v3 (retry_invalid_context) an unparseable,
    invalid or non-object answer is asked for once more before the query is
    returned without them. /v1 keeps its single call."""

    def test_retry_once_then_succeed(self, pipeline):
        steps = []
        inner = _mock_call_by_step(steps)
        ctx_calls = {"n": 0}

        def flaky(prompt, *a, **k):
            if k.get("step_name") == "contextual_environment":
                ctx_calls["n"] += 1
                if ctx_calls["n"] == 1:
        ...
        with patch.object(
            pipeline, "_call_model_async", side_effect=_as_async_mock(flaky)
        ):
            result = pipeline.run_v2(
                "elevated psa in last 3 years",
                enable_retrieval_signals=True,
        ...
        assert ctx_calls["n"] == 2 and steps.count("contextual_environment") == 1
        intent = result["intents"][0]
        assert intent["final_candidates"][0]["retrieval_signals"]["record_types"]
        # both calls are accounted for
        assert result["usage_metadata"]["contextual_environment"] == {
            k: 2 * v for k, v in USAGE.items()
        }

    def test_two_failures_return_empty_context(self, pipeline):
        def broken(prompt, *a, **k):
            if k.get("step_name") == "contextual_environment":
                return json.dumps({"temporal_by_intent": "nope"}), USAGE
            return _mock_call_by_step()(prompt, *a, **k)

        with patch.object(
            pipeline, "_call_model_async", side_effect=_as_async_mock(broken)
        ) as call:
            result = pipeline.run_v2(
                "elevated psa in last 3 years",
                enable_retrieval_signals=True,
        ...
        ctx = [
            ...
            if c.kwargs.get("step_name") == "contextual_environment"
        ]
        assert len(ctx) == 2
        assert result["record_type_matches"] == []
        # the response keeps its shape; only the signals are empty
        assert result["intents"][0]["final_candidates"] == [] or all(
            not fc["retrieval_signals"]["record_types"]
            for fc in result["intents"][0]["final_candidates"]
        )


class TestContextualEnvironmentSingleCallByDefault:
    def test_without_retry_one_call_and_its_usage_as_returned(self, pipeline):
        """/v1: one contextual-environment call, as it always made; an
        unusable answer returns the empty context straight away."""
        calls = []
        ...
        with patch.object(
            ...
            result = pipeline.run_v2(
                "elevated psa in last 3 years", enable_retrieval_signals=True
            )
        assert len(calls) == 1
        assert result["usage_metadata"]["contextual_environment"] == USAGE
        assert result["record_type_matches"] == []

    def test_non_object_answer_is_retried(self, pipeline):
        answers = iter(["[1, 2]", CONTEXT_JSON])
        inner = _mock_call_by_step()

        def llm(prompt, *a, **k):
            if k.get("step_name") == "contextual_environment":
                return next(answers), USAGE
            return inner(prompt, *a, **k)

        with patch.object(
            pipeline, "_call_model_async", side_effect=_as_async_mock(llm)
        ):
            result = pipeline.run_v2(
                "elevated psa in last 3 years",
                enable_retrieval_signals=True,
        ...
        fc = result["intents"][0]["final_candidates"][0]
        assert fc["retrieval_signals"]["record_types"]
